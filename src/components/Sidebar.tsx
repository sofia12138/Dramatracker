...
export default function Sidebar() {
  ...
  const userInitial = user?.name?.charAt(0) || user?.username?.charAt(0)?.toUpperCase() || 'U';
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 group ${
                    isActive
                      ? 'bg-primary-accent-bg text-primary-accent border border-primary-accent-border font-medium'
                      : 'text-primary-text-secondary hover:bg-primary-card hover:text-primary-text'
                  }`}
                >
                  <span className={isActive ? 'text-primary-accent' : 'text-primary-text-muted group-hover:text-primary-text-secondary'}>
                    {item.icon}
                  </span>
      ...
      {user && (
        <div className="p-4 border-t border-primary-border">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-primary-accent flex items-center justify-center shrink-0">
              <span className="text-white text-sm font-medium">{userInitial}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-primary-text truncate">{user.name || user.username}</p>
              <p className="text-xs text-primary-text-muted">{ROLE_LABELS[user.role] || user.role}</p>
            </div>
            <button
              onClick={logout}
              title="退出登录"
              className="p-1.5 rounded-lg text-primary-text-muted hover:text-red-500 hover:bg-red-50 transition-colors shrink-0"
            >
