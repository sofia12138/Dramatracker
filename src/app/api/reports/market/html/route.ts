...
import { renderMarketReportHtml } from '@/lib/report-html';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const html = renderMarketReportHtml(report);

    const platform = filters.platform && filters.platform !== 'all' ? `-${filters.platform}` : '';
    const filename = `market-insight${platform}-${filters.startDate}_to_${filters.endDate}.html`;
