...
import { buildHotAnalysisReport, type ReportFilters } from '@/lib/report-analysis';
import { renderHotReportHtml } from '@/lib/report-html';
...
function parseFilters(params: URLSearchParams): ReportFilters | { error: string } {
  ...
  if (!startDate || !endDate) {
    return { error: '缺少必要参数：startDate 和 endDate 不能为空' };
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const html = renderHotReportHtml(report);

    const platform = filters.platform && filters.platform !== 'all' ? `-${filters.platform}` : '';
    const filename = `hot-analysis${platform}-${filters.startDate}_to_${filters.endDate}.html`;
